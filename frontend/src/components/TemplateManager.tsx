...
import { ScreeningTemplate, ExtendedScreeningCriteria, TemplateHistory } from '@/types'
import { 
  PlusIcon, 
  StarIcon, 
  TrashIcon, 
...
interface TemplateManagerProps {
  ...
  currentCriteria: ExtendedScreeningCriteria
  onTemplateSelect: (template: ScreeningTemplate) => void
  onTemplateSave: (template: Omit<ScreeningTemplate, 'id' | 'created_at' | 'updated_at'>) => void
  onTemplateDelete: (templateId: string) => void
  onHistoryAdd?: (prompt: string, criteria: any) => void
}
...
const DEFAULT_TEMPLATES: ScreeningTemplate[] = [
  {
    id: 'default',
    name: '默认模板',
    description: '基础股票筛选模板',
    template: `今日{start_time}至{end_time}特大单净额排名行业前{large_order_ranking}或今日{start_time}至{end_time}特大单净额排名行业前{large_order_alt_ranking}%；今日竞价分时涨跌幅大于{bid_amp_min}小于{bid_amp_max}；今日{start_time}至{end_time}均价/开盘价大于{avg_price_ratio_min}；今日{end_time_minus_1}至{end_time}最低价/今日{end_time_minus_1}至{end_time}均价大于{low_avg_ratio_min}；今日{mid_time}收盘价/今日{start_time}至{mid_time}最高价大于{close_high_ratio_min}；今日{end_time}量比大于{volume_ratio_min}；（今日{end_time}量比/今日{end_time_minus_1}量比）-（今日{end_time_minus_1}量比/今日{mid_time}量比）*0.95＞{volume_ratio_change_min}＜{volume_ratio_change_max}；今日前1个交易日换手率/今日前3个交易日换手率＜{turnover_prev_1_over_prev_3_max}且今日前3个交易日换手率/今日前120个交易日日均换手率＜{turnover_prev_3_over_prev_120_max}；今日{end_time}换手率大于{current_turnover_min_pct}%小于{current_turnover_max_pct}%；今日{start_time}至{end_time}特大单净额大于{large_order_net_min}万；（今日{end_time_minus_1}至{end_time}特大单净额-今日{start_time}至{end_time_minus_1}特大单净额）＞{large_order_net_delta_min}万；今日前10个交易日成交均价/今日前20个交易日成交均价大于{avg_price_10_over_20_min}；今日前2个交易日振幅小于{prev_2_days_amplitude_max}；今日前3个交易日非一字线非T字线；主板非ST且市值小于{market_cap_limit}亿`,
    criteria: {
      start_time: '09:30',
      end_time: '09:33',
  ...
  },
  {
    id: 'aggressive',
    name: '激进筛选',
    description: '高量比、大振幅的激进筛选策略',
    template: `今日{start_time}至{end_time}特大单净额排名行业前{large_order_ranking}；今日竞价分时涨跌幅大于{bid_amp_min}小于{bid_amp_max}；今日{end_time}量比大于{volume_ratio_min}；今日{end_time}换手率大于{current_turnover_min_pct}%小于{current_turnover_max_pct}%；今日{start_time}至{end_time}特大单净额大于{large_order_net_min}万；主板非ST且市值小于{market_cap_limit}亿`,
    criteria: {
      start_time: '09:30',
      end_time: '09:35',
      large_order_net_amount_min: 5000000,
      large_order_ranking: 10,
      bid_amplitude_min: 2,
      bid_amplitude_max: 8,
      avg_price_ratio_min: 1.01,
      low_avg_ratio_min: 0.98,
      close_high_ratio_min: 0.98,
      volume_ratio_min: 5,
